    ...
    return (
      <div className="bg-slate-800/95 backdrop-blur-xl border border-slate-700 rounded-xl p-3 shadow-2xl">
        ...
        <p className="text-slate-400 text-sm">
          {payload[0].payload.percentage}% of portfolio
        </p>
...
export default function Portfolio() {
  ...
  const totalValue = portfolio.reduce(
    (sum, h) => sum + h.shares * (h.current_price || h.avg_cost),
    0
  );
  ...
  const chartData = portfolio.map((h, i) => ({
    ...
    percentage: (
      ((h.shares * (h.current_price || h.avg_cost)) / (totalValue || 1)) *
      100
    ).toFixed(1),
  }));
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          ...
          >
            <Card
              className={`bg-slate-900/50 border-slate-800/50 backdrop-blur-xl p-6 ${
                totalPL >= 0
            ...
            >
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <span
                  className={`text-sm ${
                    totalPL >= 0 ? "text-emerald-400/70" : "text-rose-400/70"
                  }`}
                >
                  ({totalPLPercent >= 0 ? "+" : ""}
          ...
          <div className="lg:col-span-2 space-y-4">
            <AnimatePresence mode="popLayout">
              {portfolio.map((holding, index) => {
                const currentValue =
                  holding.shares * (holding.current_price || holding.avg_cost);
