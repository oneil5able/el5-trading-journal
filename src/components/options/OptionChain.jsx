...
export default function OptionChain() {
  ...
  const generate = () => {
    ...
    const strs = strikes
      ...
      .map((s) => Number(s.trim()))
      .filter(Boolean);
    const generated = optionChain.buildOptionChain({
      underlying,
      expirations: exps,
      strikes: strs,
      type: "C",
    ...
    <Card className="luxury-card p-4">
      ...
      <div className="grid grid-cols-3 gap-3 mb-3">
        ...
        <Input
          ...
          placeholder="Expirations (YYYY-MM-DD,comma)"
        />
        <Input
          value={strikes}
          onChange={(e) => setStrikes(e.target.value)}
          placeholder="Strikes (comma separated)"
        />
      </div>
      <div className="flex gap-3 mb-3">
        <Button className="luxury-btn" onClick={generate}>
          Generate Call Chain
        </Button>
      </div>
      ...
      {chain.length > 0 && (
        <div className="grid grid-cols-3 gap-2">
          {chain.map((c) => (
            <div
              key={c.symbol}
              className="p-2 bg-[#071018] border border-[#10202a] rounded"
            >
