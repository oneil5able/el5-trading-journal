  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-slate-800/95 backdrop-blur-xl border border-slate-700 rounded-xl p-3">
        ...
        <p className={`text-sm font-semibold ${data.winRate >= 50 ? 'text-emerald-400' : 'text-rose-400'}`}>
...
export default function PsychChart({ trades }: Props) {
  ...
  const emotionData = closedTrades.reduce((acc, trade) => {
    ...
    if (!acc[trade.emotion_entry]) acc[trade.emotion_entry] = { wins: 0, total: 0 };
    ...
    <Card className="bg-slate-900/50 border-slate-800/50 backdrop-blur-xl p-6 space-y-4">
      <div className="flex items-center gap-3 mb-4">
        <div className="p-2 rounded-lg bg-purple-500/10">
          {mode === 'emotion' ? <Brain className="w-5 h-5 text-purple-400" /> : <ScatterPlot className="w-5 h-5 text-purple-400" />}
        </div>
        <div>
          <h3 className="text-white font-semibold">{mode === 'emotion' ? 'Emotion Analysis' : 'ERS vs P&L'}</h3>
          <p className="text-slate-400 text-sm">
            {mode === 'emotion' ? 'Win rate by emotional state' : 'Performance by ERS'}
          </p>
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-4">
        <button
          ...
          className={`px-3 py-1 rounded ${mode === 'emotion' ? 'bg-purple-700' : 'bg-gray-700'}`}
        >
          Emotion Chart
        </button>
        <button
          onClick={() => setMode('ers')}
          className={`px-3 py-1 rounded ${mode === 'ers' ? 'bg-purple-700' : 'bg-gray-700'}`}
        >
          ERS Chart
        </button>
      </div>
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={emotionChartData}>
                <XAxis dataKey="emotion" axisLine={false} tickLine={false} tick={{ fill: '#94a3b8', fontSize: 11 }} />
                <YAxis axisLine={false} tickLine={false} tick={{ fill: '#94a3b8', fontSize: 11 }} tickFormatter={(v: any) => `${v}%`} />
            ...
            <div className="mt-4 p-4 rounded-lg bg-slate-800/50">
              ...
              <p className="text-emerald-400 text-lg font-semibold capitalize">
                {emotionChartData[0].emotion} ({emotionChartData[0].winRate.toFixed(1)}% win rate)
              </p>
            </div>
          </div>
        ) : (
