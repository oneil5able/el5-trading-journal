      ...
      <div className="flex items-start justify-between">
        ...
        <div
          className={`p-3 rounded-xl ${
            positive !== false ? "bg-emerald-500/10" : "bg-rose-500/10"
          } group-hover:scale-110 transition-transform duration-300`}
        >
          <Icon
            className={`w-5 h-5 ${
              positive !== false ? "text-emerald-400" : "text-rose-400"
            }`}
          />
        </div>
      </div>
...
export default function PortfolioSummary({ trades = [], portfolio = [] }) {
  ...
  const portfolioValue = portfolio.reduce(
    (sum, p) => sum + p.shares * (p.current_price || p.avg_cost),
    0
  );
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
      <StatCard
        ...
        change={
          totalPL >= 0
            ? `+${((totalPL / (portfolioCost || 1)) * 100).toFixed(2)}%`
            : `${((totalPL / (portfolioCost || 1)) * 100).toFixed(2)}%`
        }
        icon={DollarSign}
        positive={totalPL >= 0}
