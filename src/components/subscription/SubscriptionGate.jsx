  ...
  return (
    <div className="min-h-screen flex items-center justify-center">
      <div className="bg-shell-card p-8 rounded-lg shadow-xl border border-shell-accent text-center">
        ...
        <p className="mb-4 text-shell-text/80">
          A subscription is required to use this app. Subscribe to continue.
        </p>
...
export default function SubscriptionGate({ children }) {
  ...
  const handleSubscribe = async () => {
    try {
      ...
      console.warn(
        "subscription server unavailable, enabling mock subscription"
      );
      localStorage.setItem("subscription_active", "1");
      setActive(true);
    }
  ...
  useEffect(() => {
    ...
    if (sessionId && !active) {
      (async () => {
        try {
          const r = await fetch(
            `/api/checkout-session?sessionId=${encodeURIComponent(sessionId)}`
          );
